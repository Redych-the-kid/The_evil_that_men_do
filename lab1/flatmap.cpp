...
FlatMap::FlatMap() : capacity(0), size(1) {
    val = new Value[size];
    keys = new Key[size];
}
...
FlatMap::FlatMap(const FlatMap &b) : size(b.size), capacity(b.capacity){
    val = new Value[size];
    keys = new Key[size];
...
bool FlatMap::contains(const Key &key) const {
    ...
    return bin_search(key) != UINT64_MAX;
}
...
Value &FlatMap::operator[](const Key &k) {
    ...
    Value v = {0, 0};
    insert(k, v);
    return val[bin_search(k)];
}

void FlatMap::set(size_t i, const Key &key, const Value &value) {
    val[i] = value;
    keys[i] = key;
}
...
size_t FlatMap::bin_search(const Key &key) const {
    size_t id = bin_util(key);
    if (keys[id] == key) {
        return id;
    }
...
FlatMap::FlatMap(FlatMap &&b) : keys(b.keys), val(b.val), capacity(b.capacity), size(b.size) {
    b.keys = nullptr;
    b.val = nullptr;
}
